...
function updateProjectImages(lang) {
  document.querySelectorAll('img[data-src-pt][data-src-en]').forEach((image) => {
    image.src = lang === 'en' ? image.dataset.srcEn : image.dataset.srcPt;

    if (image.dataset.altPt && image.dataset.altEn) {
      image.alt = lang === 'en' ? image.dataset.altEn : image.dataset.altPt;
    }
  });
}
...
langButton.addEventListener('click', () => {
  const nextLanguage = root.dataset.lang === 'pt' ? 'en' : 'pt';
